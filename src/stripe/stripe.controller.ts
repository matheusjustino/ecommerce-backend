...
export class StripeController {
	...
	public async createChargeWithCreditCard(
		@Body() body: ChargeCustomerModel,
		@User() user,
		@Res() res,
	) {
		const charge = await this.stripeService.chargeWithCreditCard(
			body,
	...
	public async getCustomerCharges(@User() user, @Res() res) {
		const customerCharges = await this.stripeService.getCustomerCharges(
			user.stripeCustomerId,
		);
	...
	public async createAndAddCardToCustomer(
		@Body() body: CreateCardTokenModel,
		@User() user,
		@Res() res,
	) {
		const customerCardToken =
			await this.stripeService.createAndAddCardToCustomer(
				body,
				user.stripeCustomerId,
			);
