...
// CORREIOS
import { CalculateShippingAndDeadlineResponseModel, CalculateShippingModel, CalculateShippingResponseModel } from '@shared/src/correios/correiosModel';
...
export class CheckoutService implements ICheckoutService {
	...
	public async setBillingShippingAddress(cartId: string, billing: BillingAddressModel, shipping: ShippingAddressModel): Promise<CartDocument> {
		const data = {
	...
	public async setShippingMethod(data: SetShippingMethod): Promise<CartDocument> {
		const cart = await this.cartService.getCartById(data.cartId);

		const shippingCostAndDeadline = await this.calculateShippingAndDeadline(data);

		const updatedCart = Object.assign(
			cart,
			{
				shippingMethod: {
	...
	public async calculateShipping(shippingMethod: SetShippingMethodBody): Promise<CalculateShippingResponseModel[]> {
		...
		return await this.correiosService.calculateShipping(calculateShippingData);
	}

	public async calculateShippingAndDeadline(shippingMethod: SetShippingMethodBody): Promise<CalculateShippingAndDeadlineResponseModel[]> {
		const calculateShippingData = {
			...this.CalculateShippingDataDefault,
			nCdServico: ShippingMethodCode[shippingMethod.shippingMethod],
			sCepDestino: shippingMethod.zip
		...
		return await this.correiosService.calculateShippingAndDeadline(calculateShippingData);
	}
}
